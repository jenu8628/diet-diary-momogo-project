    ...
    <v-app id="inspire">
      ...
      >
        ...
        <thead>
          <tr>
            <th class="tg-hrrh">
              <h2>{{month}}월 {{day}}일 총 섭취량 : {{breakfastCal + lunchCal + dinnerCal + dessertCal}} Kcal </h2>
            </th>
          </tr>
        </thead>
        <tbody>
...
export default {
  ...
  data() {
    return {
      year : this.$route.query.title,
      month : this.$route.query.body,
      day : this.$route.query.foot,
      food : 0,
    ...
    getFood (){
      ...
      .then((res)=>{
        ...
        for (let i = 0; i < res.data.length; i++){
          if (res.data[i].created_at.slice(0,4) === this.year && parseInt(res.data[0].created_at.slice(5,7)) === this.month && parseInt(res.data[0].created_at.slice(8,10)) == this.day){
            for (let j=0; j<4; j++){
